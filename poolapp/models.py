...
def create_user_profile(sender, instance, created, **kwargs):
    ...
    if created:
        data = Pool.objects.filter(id=instance.id)
        print(data)
        pk = instance.id
        print(type(pk))
        print(pk)
        ...
        # ltm.hour
        # ltm.minute
        schedule, created = CrontabSchedule.objects.get_or_create(hour=14, minute=6)
        task = PeriodicTask.objects.create(crontab=schedule, name="task_scheduled_" + unique_id,
                                           task='poolapp.tasks.time_over',
                                           args=json.dumps((pk,)))  # args takes tuple arguments like this (a,b)
        print("celery set-------")
